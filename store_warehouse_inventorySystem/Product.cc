...
const string& Product::getName() { return this->name; }
StoreLocation* Product::getStoreLocation() const { return storeLocation; }
void Product::setStoreLocation(StoreLocation* sLoc){ storeLocation = sLoc; }
...
/* We take as much as we can from the storeLocation.
   ...
   as output. 
*/
void Product::getFromStoreLocation(int& quantity) const{
...
/* 
   ...
   as output. 
*/
void Product::getFromWHLocations(int& quantity) const{
	WHLocation* loc;
	while(quantity>0){
